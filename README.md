...
To perform this QC, photos will be taken of the PCBs/anodes that will be analyzed to extract the required parameters for the analysis
...
First we read a photo for the analysis (adjust to the location of your photos)
```python
...
Since the light can be different in different regions of the photo, we will divide the photo in small sections to have more homogenous results in all the photo. To do that, you can use the following function
```python
...
```
We then compute the refined distance using the average hole color of that section and we make an histogram to identify the required cut to isolate the copper regions. The funtion ```get_max_h(h,0.25)``` can be used to find the maximum which corresponds to the copper region, the 0.25 is used to avoid the region of the histogram close to 0. Then the cut is found using ```get_selection(d_refined,h_max,2.50)``` the 2.5 corresponds to 2.5 sigma on the left with respect to the maximum, which works for most of the cases, but 2.5 sigma could be adjusted if needed, just make a test and verify that it's working
```python
...
Then we apply the selection to convert the section into a binary image, 1 corresponding to the hole and 0 to the copper, in that way we can isolate the holes for analysis. We can plot to make sure that all worked fine
```python
d_s = d_refined .< c
heatmap(d_s)
```
...
If the binary image looks ok, we can process it and obtain the holes info by using the functions:
```python
...
s_result[:,3]/calib
```
The values should be close to 2.4 mm 
Once we have verified that all worked fine, we can apply the same procedure but for the whole image, which will take about 10  min or more depending on your machine.
```python
d_selection = []
